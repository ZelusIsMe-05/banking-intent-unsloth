# Banking Intent Classification với Unsloth + LoRA

Fine-tuning mô hình ngôn ngữ lớn **Qwen3-4B-Base** bằng kỹ thuật **LoRA (Low-Rank Adaptation)** và thư viện **Unsloth** để phân loại ý định (intent) của khách hàng ngân hàng trên tập dữ liệu **Banking77**.

## 1. Tổng quan

Dự án thực hiện **Instruction Fine-tuning** mô hình `Qwen3-4B-Base` để phân loại 77 loại ý định của khách hàng ngân hàng. Thay vì huấn luyện lại toàn bộ mô hình, chúng ta sử dụng **LoRA** — chỉ thêm một lượng nhỏ tham số có thể học — giúp giảm đáng kể bộ nhớ GPU và thời gian huấn luyện. Thư viện **Unsloth** tối ưu thêm tốc độ forward/backward pass lên đến 2x so với cài đặt gốc.
...
| ------------------------------- | ----- | -------------- |
| "Card swallowed by ATM"         | 11    | card_swallowed |
| "I need to change my PIN"       | 5     | change_pin     |
| "What are your exchange rates?" | 25    | exchange_rate  |
...
| `lora.alpha`             | `32`                                                            | Hệ số scaling LoRA (thường = 2×r)                                   |
...
| `max_seq_length`  | `256`                             | Phải khớp với max_seq_length lúc training |
...
```python
!git clone https://github.com/ZelusIsMe-05/Lab02---Fine-tuning-with-dataset-Banking77.git
%cd Lab02---Fine-tuning-with-dataset-Banking77
```
...
```python
source_dir = "/content/Lab02---Fine-tuning-with-dataset-Banking77/checkpoints"
...
colab_dest_dir = "/content/Lab02---Fine-tuning-with-dataset-Banking77/checkpoints"
...
Thầy có thể truy cập link chứa video demo được em gắn ở đây:
